...
for i = 1 : Sims
    ...
    ystar = diag(diag(D0).^(-.5)) * (z - reshapedmu);
    S = ystar*ystar';
    canidate = wishrnd(S, wishartDf);
    canD0 = diag(diag(canidate).^(.5));
    canR = diag(diag(canD0).^(-1)) * canidate * diag(diag(canD0).^(-1));
    mhprob = min(0, .5*(CorrMatrixDimension + 1) *  (det(canR) - det(R0)));
    if lu(i) < mhprob
        accept = accept + 1;
        R0 = canR;
        D0 = canD0;

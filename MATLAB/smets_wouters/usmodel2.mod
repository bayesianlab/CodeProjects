...
ctrend0 ctrend1 ctrend2 ctrend3 
...
// wage markup eqn
	      ...
	      r =  crpi*(1-crr)*pinf
               ...
               +crr*r(-1)
               +ms  ; 
// empirical monetary policy function
...
ctrend0,0.3982,0.1,0.8,NORMAL_PDF,0.4,0.10;
cgy,0.05,0.01,2.0,NORMAL_PDF,0.5,0.25;
ctrend0,0.3982,0.1,0.8,NORMAL_PDF,0.4,0.10;
ctrend1,0.3982,0.1,0.8,NORMAL_PDF,0.4,0.10;
ctrend2,0.3982,0.1,0.8,NORMAL_PDF,0.4,0.10;
ctrend3,0.3982,0.1,0.8,NORMAL_PDF,0.4,0.10;
calfa,0.24,0.01,1.0,NORMAL_PDF,0.3,0.05;
end;

varobs dy dc dinve labobs pinfobs dw robs;

//estimation(optim=('MaxIter',200),datafile=usmodel_data,mode_compute=0,mode_file=usmodel_mode,first_obs=71,presample=4,lik_init=2,prefilter=0,mh_replic=250000,mh_nblocks=1,mh_jscale=0.345,mh_drop=0.2,bayesian_irf,irf=20,moments_varendo,filtered_vars,smoother);
estimation(optim=('MaxIter',200),datafile=usmodel_data,mode_compute=0,first_obs=71,presample=4,prefilter=0,mh_replic=250000,mh_nblocks=1,mh_jscale=0.20,mh_drop=0.2,,moments_varendo,filtered_vars,smoother);

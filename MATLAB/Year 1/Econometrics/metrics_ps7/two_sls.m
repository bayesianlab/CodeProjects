...
betaTrue = [10; 3];
mcSims = 1000;
store_results_z_weak = zeros(length(mcSims),1);
store_CI = zeros(length(mcSims),2);
store_results_normalOLS = zeros(length(mcSims),1);
weakness_of_instrument = .25;
for index = 1:mcSims
    % IV Esitmates 
    [iv, resid, ols_est, CI] = iv_sim(mu,sigma, rows, columns,betaTrue,...
        weakness_of_instrument);
    store_results_z_weak(index,1) = iv(1,1);
    store_CI(index,1:2) = CI;
    % OLS Esitmates
    store_results_normalOLS(index, 1) = ols_est(1,1);
end
...
str = sprintf(['True Beta is %.2f'], betaTrue(1,1))
title(str)
% Confidence Interval
figure
...
hold on
plot(1:mcSims, ones(1,mcSims)*betaTrue(1,1))
...
coverage_prob = 1 - cnt/mcSims;
str = sprintf(['The coverage probability (percent of time)\n the true',...
    ' beta value was in the interval\n is: \n %.3f percent'],...
    100*coverage_prob);
disp(str)

% The weak instruments performed as expected having larger standard 
% errors than ols and also show signs of bias. The coverage probabilities
% show the estimates are not very accurate. 

...
world <- ggplot() + mapo +
  ...
  no_back + 
  theme(panel.background=element_rect(fill="royalblue"), legend.key=element_rect(fill="grey30"))
world
...
eggs <- aggregate(finaldat[, c("onBoardDailySpend")], by=list(finaldat$EGSS_OVERALL), mean)
eggsV <- aggregate(finaldat[, c("onBoardDailySpend")], by=list(finaldat$EGSS_OVERALL), sd)
eggs <- cbind(eggs, eggsV$x)
colnames(eggs) <- c("Rating", "AvgSpend", "SD")
eggs$Rating <- factor(eggs$Rating) 
ggplot(eggs, aes(Rating, AvgSpend, fill=Rating)) + geom_bar(stat="identity")
...
colnames(tradeSpend) <- c("Trade", "AvgSpend")
tradeSpend$Trade <- reorder(tradeSpend$Trade, tradeSpend$AvgSpend)
ggplot(tradeSpend, aes(x=Trade, y= AvgSpend, fill=Trade)) + geom_bar(stat="identity") +
  coord_flip() + scale_fill_discrete(guide = guide_legend(reverse=TRUE))

...
def train_model(model_path, simulations_training, simulations_validation, actuals_training,actuals_validation, batch_size=32, 
    ...
    layer1 = keras.layers.Dense(units=100, kernel_initializer='glorot_uniform', activation='relu')(inputA)
    do1 = keras.layers.Dropout(rate=.1)(layer1)
    hidden1 = keras.layers.Dense(units=100, kernel_initializer='glorot_uniform', activation='relu')(do1)
    do2 = keras.layers.Dropout(rate=.1)(hidden1)
    hidden2 = keras.layers.Dense(units=100, kernel_initializer='glorot_uniform', activation='relu')(hidden1)
    do3 = keras.layers.Dropout(rate=.1)(hidden2)
    hidden3 = keras.layers.Dense(units=100, kernel_initializer='glorot_uniform', activation='relu')(do3)
    do4 = keras.layers.Dropout(rate=.1)(hidden3)
    hidden4 = keras.layers.Dense(units=100, kernel_initializer='glorot_uniform', activation='relu')(do4)
    do5 = keras.layers.Dropout(rate=.1)(hidden4)

    out = Dense(1, activation='sigmoid')(do5)
...
if __name__=='__main__':
    ...
    train_model('C:\\Users\\dflanner\\qed\\Aspen\\MarginOptimization\\dev\\hptuning\\wide_deep_relu_dropout\\', simulations_training, simulations_validation, actuals_training, actuals_validation)

...
# %%
def train_model(model_path, simulations_training, simulations_validation, actuals_training,actuals_validation, batch_size=32, 
                epoch_cnt=30, learning_rate=1e-4):

    drop_col = ['SimulatedWinBit','ObjectiveWt', \
                'WinAboveObjectiveWt','WinBelowObjectiveWt',
                'LossAboveObjectiveWt','LossBelowObjectiveWt']
    ...
    layer1 = keras.layers.Dense(units=10, kernel_initializer='glorot_uniform', activation='relu')(inputA)
    bn = keras.layers.BatchNormalization()(layer1)
    hidden2 = keras.layers.Dense(units=10, kernel_initializer='glorot_uniform', activation='relu')(bn)
    bn2 = keras.layers.BatchNormalization()(hidden2)
    hidden3 = keras.layers.Dense(units=10, kernel_initializer='glorot_uniform', activation='relu')(bn2)
    bn3 = keras.layers.BatchNormalization()(hidden3)
    hidden4 = keras.layers.Dense(units=10, kernel_initializer='glorot_uniform', activation='relu')(bn3)
    bn4 = keras.layers.BatchNormalization()(hidden4)
    hidden5 = keras.layers.Dense(units=10, kernel_initializer='glorot_uniform', activation='relu')(bn4)
    bn5 = keras.layers.BatchNormalization()(hidden5)
    hidden6 = keras.layers.Dense(units=10, kernel_initializer='glorot_uniform', activation='relu')(bn5)
    bn6 = keras.layers.BatchNormalization()(hidden6)
    hidden7 = keras.layers.Dense(units=10, kernel_initializer='glorot_uniform', activation='relu')(bn6)
    bn7 = keras.layers.BatchNormalization()(hidden7)
    hidden8 = keras.layers.Dense(units=10, kernel_initializer='glorot_uniform', activation='relu')(bn7)
    bn8 = keras.layers.BatchNormalization()(hidden8)
    out = Dense(1, activation='sigmoid')(bn8)
...
if __name__=='__main__':
    ...
    train_model('C:\\Users\\dflanner\\qed\\Aspen\\MarginOptimization\\dev\\hptuning\\narrow_deep\\', simulations_training, simulations_validation, actuals_training, actuals_validation)

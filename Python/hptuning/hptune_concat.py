...
def train_model(model_path, simulations_training, simulations_validation, actuals_training,actuals_validation, batch_size=64, 
    ...
    layer1 = keras.layers.Dense(units=500, kernel_initializer='glorot_uniform', activation='sigmoid')(inputA)
    d1 = keras.layers.Dropout(rate=.1)(layer1)
    hidden1 = keras.layers.Dense(units=500, kernel_initializer='glorot_uniform', activation='sigmoid')(d1)
    d2 = keras.layers.Dropout(rate=.1)(hidden1)
    
    concatlayer1 = keras.layers.Concatenate()([d1, d2])
    
    out = Dense(1, activation='sigmoid')(concatlayer1)
...
if __name__=='__main__':
    simulations_training, simulations_validation, actuals_training,actuals_validation=load_process_data2('insample.csv')
    train_model('C:\\Users\\dflanner\\qed\\Aspen\\MarginOptimization\\dev\\hptuning\\concat\\', simulations_training, simulations_validation, actuals_training, actuals_validation)

...
public:
                            ...
                            const MatrixXd &G0)
    {
        ...
        // MatrixXd S2newinv = S2new.llt().solve(IT);
        // MatrixXd S2newinvlower = S2newinv.llt().matrixL();
        // MatrixXd S2oldinv = S2old.llt().solve(IT);
        ...
        double lalpha = min(0.0, logmvnpdf(epsilons.leftCols(lags), zeros, P1) - logmvnpdf(epsilons.leftCols(lags), zeros, P0));

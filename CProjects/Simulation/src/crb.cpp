...
void Crb::fillfzStar(VectorXd& fzStar, int redRuns, VectorXd& a, VectorXd& b, MatrixXd& sample,
...
void Crb::chibRaoBlackwell(VectorXd& a, VectorXd& b, VectorXd& mu, MatrixXd& sigma, 
...
void Crb::ml(VectorXd &betas, double sigmas, VectorXd &fzStar, VectorXd &y,
             MatrixXd &X) {
  double mLike = lrLikelihood(betas, sigmas, y, X) +
                 logmvnpdf(betaPrior, sigmaPrior, betas) +
                 loginvgammapdf(sigmas, igamA, igamB) - log(fzStar.prod());
  cout << "CRB method marginal likelihood:    " << mLike << endl;
}
...
void Crb::runSim(VectorXd &betas, MatrixXd &sigma, VectorXd &y, MatrixXd &X,
  ...
  for (int i = 0; i < nSims; i++) {
    chibRaoBlackwell(ll, ul, betas, sigma, zStar, fzStar, sims, burnin, sims,
                     burnin);
    b = zStar.tail(J - 1);
    mLike(i) = lrLikelihood(b, zStar(0), y, X) +
               logmvnpdf(betaPrior, sigmaPrior, b) +
               loginvgammapdf(zStar(0), igamA, igamB) - log(fzStar.prod());
  }

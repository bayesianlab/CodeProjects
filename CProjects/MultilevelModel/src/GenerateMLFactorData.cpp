...
void DynamicFactorsArErrors::genData(const int &nObs, const int &nEqns,
                                     const int &nXs, const double &coeffValues,
  ...
  if (omArTerms.size() > 0) {
    ...
    if (nXs == 0) {
      ...
      Xt.leftCols(nXs) << MatrixXd::Ones(K * T, 1),
          normrnd(0, 1, K * T, nXs - 1);
      Xt.rightCols(levels) = packageFactorsInXt(InfoMat, Factors, K);
    } else {
      throw invalid_argument(
          "Invalid number of x's included in genOtrokData. nXs > 0.");
    }
    ...
    for (int k = 0; k < K; ++k) {
      ...
      yt.row(k).leftCols(lagsOm) =
          (Xbeta.row(k).leftCols(lagsOm).transpose() +
...
void GenerateMLFactorData::genData(int _nObs, int _nEqns,
